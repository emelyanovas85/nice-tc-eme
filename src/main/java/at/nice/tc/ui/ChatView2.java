...
import at.nice.tc.events.UserMessageEvent;
...
import at.nice.tc.ui.components.MarkdownMessageWithThinking2;
...
public class ChatView2 extends Composite<VerticalLayout> implements BeforeEnterObserver {
...
//    private final JiraService jiraService;
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        query.getSingleParameter(Constants.CHAT_ID).ifPresentOrElse(config::setChatId, () -> {
            ...
            UI.getCurrent().navigate(ChatView2.class, updated);
        });
        ...
        query.getSingleParameter("userFio").map(ChatView2::parseFio).ifPresent(config::setUserFio);
...
//        test = testjiraService.getTestWithNested("VPEPVV-T2706").join();
    ...
    private MarkdownMessageWithThinking2 actualBotMessage;
    ...
    private void onSubmit(ClickEvent<Button> buttonClickEvent) {
        ...
        actualBotMessage = new MarkdownMessageWithThinking2("Агент Jira", LocalDateTime.now());
    ...
    private void subscribeToChatStream() {
        ...
        subscription = memoryService.subscribe2(config.getChatId())
                .subscribe(token -> ui.access(() -> {
                            ...
                            actualBotMessage.processResponse(token);
                            scroll.scrollToBottom();
                        }),
                        err -> {
                            actualBotMessage.handleError(err);
                            stop();
                        },
                        () -> {
                            actualBotMessage.handleComplete();
                            stop();
                        }
                );
    }
