...
public class InitialState extends ProcessingState {
    ...
    /**
     * Если true — всегда использовать синхронный appendMarkdown (для setMarkdown, когда
     * компонент ещё не в DOM, но UI уже есть).
     */
    ...
    public InitialState(MarkdownMessageWithThinking context, boolean forceSync) {
        super(context);
    ...
    public ProcessingState process(String chunk) {
        ...
        if (firstTag.isPresent()) {
            ...
            return switch (firstTag.get().tag()) {
                ...
                default -> {
                    ...
                    buffer.delete(minPos, minPos + TOOL_OPEN.length());
                    yield new InitialState(context, forceSync).process(buffer.toString());
                }
            };
        }
        ...
        buffer.setLength(0);
        return new MainState(context, forceSync);
    }

    private void sendToMain(String text) {
        final MarkdownMessage mainMessage = context.getMainMessage();
        if (forceSync) {
            // Полный текст сетается до добавления в DOM — всегда синхронно
            mainMessage.appendMarkdown(text);
        } else {
            checkUiAccessed(isAccessed -> {
                if (isAccessed)
                    mainMessage.appendMarkdownAsync(text);
                else
                    mainMessage.appendMarkdown(text);
            });
        }
